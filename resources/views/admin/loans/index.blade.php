<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('admin.loans.index') }}" method="GET" class="mb-4">
                <div class="flex gap-2">
                    <input type="text" name="q" value="{{ $q ?? '' }}"
                        placeholder="Cari nama peminjam atau judul buku..." class="w-full border rounded p-2">
            ...
            @if(session('error'))
            <div class="mb-4 p-3 bg-red-100 text-red-800 rounded">{{ session('error') }}</div> @endif
                ...
                <table class="min-w-full divide-y">
                    ...
                    <tbody>
                        @forelse($loans as $loan)
                            <tr class="border-t">
                                <td class="px-4 py-2">{{ $loop->iteration + ($loans->currentPage() - 1) * $loans->perPage() }}
                                </td>
                                <td class="px-4 py-2">{{ $loan->user->name }}<br><small>{{ $loan->user->email }}</small>
                                </td>
                                ...
                                <td class="px-4 py-2 text-right space-x-2">
                                    @if($loan->status === 'pending')
                                        <form action="{{ route('admin.loans.approve', $loan) }}" method="POST" class="inline">
                                            @csrf
                                            <button onclick="return confirm('Setujui permintaan ini?')"
                                                class="px-3 py-1 bg-green-600 text-black rounded">Approve</button>
                                        </form>
                                        <form action="{{ route('admin.loans.reject', $loan) }}" method="POST" class="inline">
                                            @csrf
                                            <button onclick="return confirm('Tolak permintaan ini?')"
                                                class="px-3 py-1 bg-red-600 text-black rounded">Tolak</button>
                                        </form>
                                    @elseif($loan->status === 'dipinjam')
                                        <form action="{{ route('admin.loans.return', $loan) }}" method="POST" class="inline">
                                            @csrf
                                            <button onclick="return confirm('Tandai sudah dikembalikan?')"
                                                class="px-3 py-1 bg-blue-600 text-black rounded">Return</button>
                                        </form>
                                    @else
